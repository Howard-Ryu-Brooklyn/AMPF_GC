...
#define ROS_MEAS_UNITS sl::UNIT::METER
        ...
        mGnssFixQos(1)
  {
        ...
        (ZED_SDK_MAJOR_VERSION == SDK_MAJOR_REQ && ZED_SDK_MINOR_VERSION < SDK_MINOR_REQ))
    {
      RCLCPP_ERROR_STREAM(
          get_logger(),
          "This version of the ZED ROS2 wrapper is designed to work with ZED SDK v" << static_cast<int>(SDK_MAJOR_REQ) << "." << static_cast<int>(SDK_MINOR_REQ) << " or newer.");
      RCLCPP_INFO_STREAM(
          get_logger(), "* Detected SDK v" << ZED_SDK_MAJOR_VERSION << "." << ZED_SDK_MINOR_VERSION << "." << ZED_SDK_PATCH_VERSION << "-" << ZED_SDK_BUILD_ID);
  ...
  std::string ZedCamera::getParam(std::string paramName, std::vector<std::vector<float>> &outVal)
  {
    ...
    {
      RCLCPP_WARN_STREAM(
          get_logger(), "The parameter '" << paramName << "' is not available or is not valid, using the default "
                                                          "value: []");
    }
      ...
      std::string paramName, T defValue, T &outVal, std::string log_info, bool dynamic)
  {
    ...
    {
      RCLCPP_WARN_STREAM(
          get_logger(), "The parameter '" << paramName << "' is not available or is not valid, using the default value: " << defValue);
    }
  ...
  {
    // Streaming paramterts
    if (!get_parameter("use_streaming", mStreaming))
    {
      RCLCPP_WARN_STREAM(
          get_logger(),
          "The parameter 'use_streaming' is not available or is not valid, using the default value: " << mStreaming);
    }
    if (mStreaming)
    {
      RCLCPP_INFO(get_logger(), " *** STREAMING MODE ACTIVE ***");
      getParam("streaming_address", mStreamIPAddr, mStreamIPAddr, " * Streaming address: ");
      getParam("streaming_port", mStreamPort, mStreamPort, " * Streaming port: ");
    }
      ...
      const std::vector<std::vector<float>> &in_poly, std::vector<sl::float2> &out_poly)
  {
    ...
    if (poly_size < 3)
    {
      ...
      {
        RCLCPP_WARN_STREAM(
            get_logger(), "A vector with " << poly_size << " points is not enough to create a polygon to set a Region "
                                                           "of Interest.");
        return std::string();
      }
    }
    else
    {
      for (size_t i; i < poly_size; ++i)
      {
        ...
        {
          RCLCPP_WARN_STREAM(
              get_logger(), "The component with index '" << i << "' of the ROI vector "
  ...
  void ZedCamera::getSimParams()
  {
    ...
    if (!get_parameter("use_sim_time", mSimEnabled))
    {
      RCLCPP_WARN_STREAM(
          get_logger(),
          "The parameter 'use_sim_time' is not available or is not valid, using the default value: " << mSimEnabled);
    }
    if (mSimEnabled)
    {
  ...
  void ZedCamera::getVideoParams()
  {
    ...
    if (sl_tools::isZEDX(mCamUserModel))
    {
      ...
      if (mGmslAutoExpTimeRangeMax > mCamGrabFrameRate * 1000 || mGmslAutoExpTimeRangeMax > 30000)
      {
        ...
        RCLCPP_WARN_STREAM(
            get_logger(),
            "The values of 'video.auto_exposure_time_range_max' is clamped to max(30000,'general.grab_frame_rate'x1000): " << mGmslAutoExpTimeRangeMax);
      }
      getParam(
    ...
    if (get_parameter(paramName, paramVal))
    {
      qos_reliability = paramVal.as_int() == 1 ? RMW_QOS_POLICY_RELIABILITY_RELIABLE : RMW_QOS_POLICY_RELIABILITY_BEST_EFFORT;
    ...
    if (get_parameter(paramName, paramVal))
    {
      qos_durability = paramVal.as_int() == 0 ? RMW_QOS_POLICY_DURABILITY_TRANSIENT_LOCAL : RMW_QOS_POLICY_DURABILITY_VOLATILE;
  ...
  void ZedCamera::getDepthParams()
  {
    ...
    if (!mDepthDisabled)
    {
      ...
      if (get_parameter(paramName, paramVal))
      {
        qos_durability = paramVal.as_int() == 1 ? RMW_QOS_POLICY_DURABILITY_TRANSIENT_LOCAL : RMW_QOS_POLICY_DURABILITY_VOLATILE;
  ...
  void ZedCamera::getPosTrackingParams()
  {
    ...
    {
      RCLCPP_WARN_STREAM(
          get_logger(), "The parameter '" << paramName << "' is not available or is not valid, using the default value");
  ...
  void ZedCamera::getOdParams()
  {
    ...
    if (!matched)
    {
      RCLCPP_WARN_STREAM(
          get_logger(),
          "The value of the parameter 'object_detection.model' is not valid: '" << model_str << "'. Using the default value.");
    }
    RCLCPP_INFO_STREAM(
        get_logger(),
    ...
    RCLCPP_INFO_STREAM(
        get_logger(), " * Object Filtering mode: " << filtering_mode << " - " << sl::toString(mObjFilterMode).c_str());
  ...
  void ZedCamera::getBodyTrkParams()
  {
    ...
    if (!matched)
    {
      RCLCPP_WARN_STREAM(
          get_logger(),
          "The value of the parameter 'body_tracking.model' is not valid: '" << model_str << "'. Using the default value.");
    }
    RCLCPP_INFO_STREAM(
        get_logger(),
    ...
    if (!matched)
    {
      RCLCPP_WARN_STREAM(
          get_logger(),
          "The value of the parameter 'body_tracking.body_format' is not valid: '" << fmt_str << "'. Using the default value.");
    }
    RCLCPP_INFO_STREAM(
        get_logger(),
    ...
    if (!matched)
    {
      RCLCPP_WARN_STREAM(
          get_logger(),
          "The value of the parameter 'body_tracking.body_kp_selection' is not valid: '" << body_sel_str << "'. Using the default value.");
    }
    ...
    for (const rclcpp::Parameter &param : parameters)
    {
      ...
      else if (param.get_name() == "video.brightness")
      {
        if (sl_tools::isZEDX(mCamRealModel))
        {
          RCLCPP_WARN_STREAM(
              get_logger(),
              "Parameter '" << param.get_name() << "' not available for " << sl::toString(mCamRealModel).c_str());
          break;
        }
      ...
      else if (param.get_name() == "depth.remove_saturated_areas")
      {
        ...
        RCLCPP_INFO_STREAM(
            get_logger(), "Parameter '" << param.get_name() << "' correctly set to " << (mRemoveSatAreas ? "TRUE" : "FALSE"));
      }
      ...
      else if (param.get_name() == "object_detection.mc_people")
      {
        ...
        RCLCPP_INFO_STREAM(
            get_logger(), "Parameter '" << param.get_name() << "' correctly set to " << (mObjDetPeopleEnable ? "TRUE" : "FALSE"));
      }
      else if (param.get_name() == "object_detection.mc_vehicle")
      {
        ...
        RCLCPP_INFO_STREAM(
            get_logger(), "Parameter '" << param.get_name() << "' correctly set to " << (mObjDetVehiclesEnable ? "TRUE" : "FALSE"));
      }
      else if (param.get_name() == "object_detection.mc_bag")
      {
        ...
        RCLCPP_INFO_STREAM(
            get_logger(), "Parameter '" << param.get_name() << "' correctly set to " << (mObjDetBagsEnable ? "TRUE" : "FALSE"));
      }
      else if (param.get_name() == "object_detection.mc_animal")
      {
        ...
        RCLCPP_INFO_STREAM(
            get_logger(), "Parameter '" << param.get_name() << "' correctly set to " << (mObjDetAnimalsEnable ? "TRUE" : "FALSE"));
      }
      else if (param.get_name() == "object_detection.mc_electronics")
      {
        ...
        RCLCPP_INFO_STREAM(
            get_logger(), "Parameter '" << param.get_name() << "' correctly set to " << (mObjDetElectronicsEnable ? "TRUE" : "FALSE"));
      }
      else if (param.get_name() == "object_detection.mc_fruit_vegetable")
      {
        ...
        RCLCPP_INFO_STREAM(
            get_logger(), "Parameter '" << param.get_name() << "' correctly set to " << (mObjDetFruitsEnable ? "TRUE" : "FALSE"));
      }
      else if (param.get_name() == "object_detection.mc_sport")
      {
        ...
        RCLCPP_INFO_STREAM(
            get_logger(), "Parameter '" << param.get_name() << "' correctly set to " << (mObjDetSportEnable ? "TRUE" : "FALSE"));
      }
  ...
  void ZedCamera::initPublishers()
  {
    ...
    if (!mDepthDisabled && mMappingEnabled)
    {
      ...
      RCLCPP_INFO_STREAM(
          get_logger(), "Advertised on topic " << mPubFusedCloud->get_topic_name() << " @ " << mFusedPcPubRate << " Hz");
  ...
  bool ZedCamera::startCamera()
  {
    ...
    else
    {
      ...
      if (mStreaming)
      {
        mInitParams.input.setFromStream(mStreamIPAddr.c_str(), mStreamPort);
        RCLCPP_INFO_STREAM(get_logger(), "Streaming Mode Enabled" << mStreaming);
        RCLCPP_INFO_STREAM(get_logger(), "IP: " << mStreamIPAddr.c_str() << ", Port: " << mStreamPort);
      }
      else
      {
    ...
    while (1)
    {
      ...
      {
        RCLCPP_WARN(
            get_logger(), "Error connecting to the simulator: %s", sl::toString(mConnStatus).c_str());
      }
      else
      {
    ...
    {
      RCLCPP_WARN_STREAM(
          get_logger(), "!!! `general.grab_frame_rate` value is not valid: '" << mCamGrabFrameRate << "'. Automatically replaced with '" << realFps << "'. Please fix the parameter !!!");
    ...
    if (pub_w > mCamWidth || pub_h > mCamHeight)
    {
      RCLCPP_WARN_STREAM(
          get_logger(),
          "The publishing resolution (" << pub_w << "x" << pub_h << ") cannot be higher than the grabbing resolution (" << mCamWidth << "x" << mCamHeight << "). Using grab resolution for output messages.");
      pub_w = mCamWidth;
      pub_h = mCamHeight;
    }
  ...
  bool ZedCamera::startPosTracking()
  {
    ...
    if (mGnssFusionEnabled && err == sl::ERROR_CODE::SUCCESS)
    {
      ...
      if (fus_err != sl::FUSION_ERROR_CODE::SUCCESS)
      {
        mPosTrackingStarted = false;
        RCLCPP_WARN(
            get_logger(), "Fusion Pos. Tracking not started: %s", sl::toString(fus_err).c_str());
  ...
  bool ZedCamera::start3dMapping()
  {
    ...
    {
      RCLCPP_WARN_STREAM(
          get_logger(), "'mapping.resolution' value (" << mMappingRes << " m) is lower than the allowed resolution "
    ...
    {
      RCLCPP_WARN_STREAM(
          get_logger(), "'mapping.resolution' value (" << mMappingRes << " m) is higher than the allowed resolution "
                                                                         "values. Fixed automatically");
      mMappingRes = hRes;
    }
    ...
    if (mMappingRangeMax < 0)
    {
      ...
      RCLCPP_INFO_STREAM(
          get_logger(), "Mapping: max range set to " << mMappingRangeMax << " m for a resolution of " << mMappingRes << " m");
    }
    else if (mMappingRangeMax < lRng)
    {
      RCLCPP_WARN_STREAM(
          get_logger(), "'mapping.max_mapping_range_m' value (" << mMappingRangeMax << " m) is lower than the allowed "
    ...
    {
      RCLCPP_WARN_STREAM(
          get_logger(), "'mapping.max_mapping_range_m' value (" << mMappingRangeMax << " m) is higher than the allowed "
                                                                                       "resolution values. Fixed "
                                                                                       "automatically");
        ...
        if (!mSimEnabled)
        {
          if (mGnssFusionEnabled && mGnssFixNew)
          {
            ...
            float dT_sec =
                ...
                1e9;
            DEBUG_STREAM_GNSS(
                "DeltaT: " << dT_sec << " sec [" << std::fixed << std::setprecision(9) << static_cast<float>(real_frame_ts.nanoseconds()) / 1e9 << "-" << static_cast<float>(mGnssTimestamp.nanoseconds()) / 1e9);
  ...
  void ZedCamera::publishVideoDepth(rclcpp::Time &out_pub_ts)
  {
    ...
    if (mRgbSubscribed && (mDepthSubnumber > 0 || mDepthInfoSubnumber > 0))
    {
      ...
      {
        RCLCPP_WARN_STREAM(
            get_logger(), "!!!!! DEPTH/RGB ASYNC !!!!! - Delta: " << 1e-9 * static_cast<double>(ts_depth - ts_rgb) << " sec");
      }
    }
    // <---- Check RGB/Depth sync
  ...
  void ZedCamera::processGeoPose()
  {
    ...
    DEBUG_GNSS(
        ...
        mLastEcefPose.z);
    DEBUG_GNSS(
        " * Lat. Long.: %.6f°, %.6f°,%.3f m", mLastLatLongPose.getLatitude(false), mLastLatLongPose.getLongitude(false),
    ...
    if (mMap2UtmTransfValid && mPublishUtmTf)
    {
      ...
      transformStamped.transform = mUtmAsParent ? (tf2::toMsg(mMap2UtmTransf.inverse())) : (tf2::toMsg(mMap2UtmTransf));
  ...
  void ZedCamera::processDetectedObjects(rclcpp::Time t)
  {
    ...
    if (!objects.is_new)
    { // Async object detection. Update data only if new
  ...
  void ZedCamera::processBodies(rclcpp::Time t)
  {
    ...
    if (!bodies.is_new)
    { // Async body tracking. Update data only if new
      // detection is available
    ...
    if (!mSvoMode && mFrameCount % 5 == 0)
    {
      ...
      if (mTriggerAutoExpGain)
      {
        ...
        else
        {
          ...
          DEBUG_STREAM_CTRL(
              "New setting for " << sl::toString(setting).c_str() << ": " << (mCamAutoExpGain ? 1 : 0));
        }
      }
      ...
      if (mTriggerAutoWB)
      {
        ...
        else
        {
          ...
          DEBUG_STREAM_CTRL(
              "New setting for " << sl::toString(setting).c_str() << ": " << (mCamAutoWB ? 1 : 0));
        }
      }
      ...
      std::shared_ptr<zed_interfaces::srv::SetPose_Response> res)
  {
    ...
    RCLCPP_INFO_STREAM(
        get_logger(), "New pose: [" << req->pos[0] << "," << req->pos[1] << "," << req->pos[2] << ", " << req->orient[0] << "," << req->orient[1] << "," << req->orient[2] << "]");
    ...
    if (mGrabStatus == sl::ERROR_CODE::SUCCESS)
    {
      ...
      if (isDepthRequired())
      {
        ...
        if (mPosTrackingStarted)
        {
          ...
          stat.addf(
              "Pos. Tracking status [Odometry]", "%s", sl::toString(mPosTrackingStatusCamera).c_str());
  ...
  void ZedCamera::callback_gnssFix(const sensor_msgs::msg::NavSatFix::SharedPtr msg)
  {
    ...
    DEBUG_STREAM_GNSS(
        "GNSS Ts: " << ts_gnss_part_sec / 1000000000 << " sec + " << ts_gnss_part_nsec << " nsec = " << ts_gnss_nsec << " nsec fused");
    ...
    if (!mGnssFixValid)
    {
      ...
      DEBUG_STREAM_GNSS(
          " * First valid datum - Lat: " << std::fixed << std::setprecision(9) << latit << "° - Long: " << longit << "° - Alt: " << altit << " m");
    }
    ...
    {
      DEBUG_STREAM_GNSS(
          "Datum ingested - [" << mGnssTimestamp.nanoseconds() << " nsec] " << latit << "°," << longit << "° / " << altit << " m");
      ...
      std::shared_ptr<robot_localization::srv::ToLL_Response> res)
  {
    ...
    RCLCPP_INFO(
        ...
        geo_pose.latlng_coordinates.getLatitude(false), geo_pose.latlng_coordinates.getLongitude(false), geo_pose.latlng_coordinates.getAltitude());
  }
